...
export default function Home() {
 ...
 // Q3selectionからの回答を収集する新しい関数
  ...
  const handleSubmit = async () => { 
    ...
    const invalidInputQuestions = []; // 無効な入力をした設問名を格納する配列
    ...
    // Q10とQ11について、0以下の数値が入力されているかチェック
    if (responses.q10Selection <= 0) invalidInputQuestions.push("Q10");
    if (responses.q11Selection <= 0) invalidInputQuestions.push("Q11");

    // 未選択の設問があれば警告を表示
    if (unselectedQuestions.length > 0) {
      alert(`${unselectedQuestions.join("、")}の設問が未選択です`);
      return; // 送信処理を中断
    }

    // 無効な入力があれば警告を表示
    if (invalidInputQuestions.length > 0) {
      alert(`${invalidInputQuestions.join("、")}には1以上の数値を入力してください`);
      return; // 送信処理を中断
    } 
  // 送信処理を続行
    // ここにサーバーに送信するための処理を記述
    try {
      const response = await fetch('http://127.0.0.1:8000/submit/', {
        ...
        body: JSON.stringify({
          ...
          q3_answer: responses.q3Selection, 
          q4_answer: responses.q4Selection, 
          q5_answer: responses.q5Selection,
          q6_answer: responses.q6Selection,
