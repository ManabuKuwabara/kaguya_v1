...
class Answers(BaseModel):
    ...
    q3_answer: int  # Q3 の回答を数値で受け取る
    q4_answer: int  # Q4 の回答を数値で受け取る
    q5_answer: int  # Q5 の回答を数値で受け取る
    q6_answer: int  # Q6 の回答を数値で受け取る
...
async def submit_answers(request: Request, answers: Answers):
    ...
    try:
        ...
        # 回答を answers テーブルに挿入
        conn.execute('INSERT INTO answers (goal, team_structure, solo_work, remote_work, solo_web, web_mtg, atmosphere,color, well_being, office_capacity, office_size, answer_date) VALUES (?,?,?,?,?,?,?,?, ?, ?, ?, ?)', 
             (q1_answers_str, answers.q2_answer, answers.q3_answer, answers.q4_answer, answers.q5_answer, answers.q6_answer, q7_answers_str,answers.q8_answer, q9_answers_str,answers.q10_answer,answers.q11_answer, answer_datetime,))
